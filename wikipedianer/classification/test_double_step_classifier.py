...
class DoubleStepClassifierWVTest(DoubleStepClassifierHFTest):

    TEST_FILES_DIR = os.path.join('wikipedianer', 'classification',
                                  'test_files_wv')
    WORD_VECTOR_MODEL = None

    @classmethod
    def setUpClass(cls):
        model_filename = os.path.join(cls.TEST_FILES_DIR,
                                      'generated_wordvectormodel.bin.gz')
        if os.path.exists(model_filename):
            # Read the model
            cls.WORD_VECTOR_MODEL = gensim.models.Word2Vec.load(model_filename)
        else:
            # Read sentences
            sentences_filename = os.path.join(
                cls.TEST_FILES_DIR, 'random_sentences.pickle')
            with open(sentences_filename, 'rb') as infile:
                sentences = pickle.load(infile)
            # Train a new model
            model = gensim.models.Word2Vec(sentences, size=50, window=3,
    ...
    def setUp(self):
        ...
        # Data to train. Each instance is a list with 7 elements, each
        # element is a 2-uple with the original token and the lower case token.
